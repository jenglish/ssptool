...
/** file callback: parse and strip YAML front matter, add 'data' property.
 */
var parseFrontMatter = function (file) {
    let fn = file.relative
      , contents = file.contents.toString()
      , yamlOpts = { filename: fn, schema: yaml.CORE_SCHEMA }
      , gmOpts = { engines: { yaml: fm => yaml.load(fm, yamlOpts) } }
      , matter = grayMatter(contents, gmOpts)
    ;
    file.data = matter.data;
    file.contents = Buffer.from(matter.content.trim());
};
...
var addKey = function (file) {
    var dirbasename = function (dirname) {
        var path = dirname.split('/');
        return path[path.length - 1];
    };
    if (file.basename === 'component.yaml') {
        file.key = file.data.key = file.data.key || dirbasename(file.dirname);
    } else {
        file.key = file.stem;
    }
};
...
class Loader {
    ...
    loadMarkdown (dir, pattern, cb, done) {
        ...
        exhaust(
            vfs.src(pattern, { cwd: dir, follow: true })
                .pipe(fileCallback(parseFrontMatter))
