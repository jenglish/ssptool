 ...
 * @param {Object} allowed - recognized values and synonyms
 * @param {string} fallback -  default value if missing or unrecognized
 */
function normalizeEnumerant (value, allowed, fallback) {
    return value && value in allowed ? allowed[value] : fallback;
}
function ImplementationStatus (status) {
    return normalizeEnumerant(status, {
        none: 'none',
        planned: 'planned',
        partial: 'partial',
        complete: 'complete',
...
/**
 * @return path name with trailing component removed
 */
...
class Database {
    ...
    addComponent (component) {
        ...
        for (var sat of component.satisfies || []) {
            let status = ImplementationStatus(sat.implementation_status);
            this.satisfactions.add(_.extend(sat, {
    ...
    addPage (page) {
        if (page.basename === 'README.md' && page.dirname !== page.base) {
            page.path = dirname(page.path);
        }
        this.pages.add(page);
        if (page.data.satisfies || page.data.component) {
            let component_key = page.data.component || page.stem
              , name = page.data.title
              , status = ImplementationStatus(page.data.implementation_status)
              ;

            this.components.add({
                key:    component_key,
                name:   name,
                system: page.data.system || dirname(page.relative),
                responsible_role: page.data.responsible_role,
                description: page.contents.toString(),
                page:   page,
            });
