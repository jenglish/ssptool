/** Defines webapp site structure and URI space.
 * @module
 */

const _ = require('lodash')
  , { gqlQuery } = require('./graphql.js')
...
function apppath(basedir, relative) {
...
class EntityLinks {
    ...
    page (relative)            { return apppath('pages', relative); }
    pagedir (subdir)           { return apppath('pages', subdir); }
...
function addStandards (db, site) {
    site.begin(appurl('standards'), 'Standards').topmost = true;
...
function addCertifications (db, site) {
    site.begin(appurl('certifications'), 'Certifications').topmost = true;
    db.controls.chain()
    .flatMap(control =>
...
function addComponents (db, site) {
    site.begin(appurl('components'), 'Components').topmost = true;
