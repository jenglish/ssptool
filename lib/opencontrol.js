...
System.prototype.addStandard = function (standard) {
...
System.prototype.addControl = function (key, control) {
    debug(" ... Found control: %s", key);
    this.controls[key] = control;
}

System.prototype.addComponent = function (component) {
    debug("... Found component: %s", component.key);
    this.components[component.key] = component;
}

System.prototype.addCertification = function (certification) {
 ...
 *  (typically `./opencontrols/`) into a new {@link opencontrol.System}.
 *
 *  @param cb - callback function (err, sys)
 */
var load = function (dir, done) {
    ...
    async.parallel(
       [ loadSection('standards/*.yaml', sys.addStandard.bind(sys))
       , loadSection('components/*/*.yaml', sys.addComponent.bind(sys))
       , loadSection('certifications/*.yaml', sys.addCertification.bind(sys))
       ], function (err, _unused) { done(err, sys); });
};
